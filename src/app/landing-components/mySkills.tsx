"use client"

import { useEffect, useRef } from "react";
import { gsap } from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";

gsap.registerPlugin(ScrollTrigger);

export default function MySkills() {
    const headingRef = useRef<HTMLDivElement>(null);
    const borderRef = useRef<HTMLDivElement>(null);
  
   useEffect(() => {
      if (headingRef.current && borderRef.current) {
        const letters = headingRef.current.querySelectorAll(".heading-char");
  
        const tl = gsap.timeline({
          scrollTrigger: {
            trigger: headingRef.current,
            start: "top 85%",
            toggleActions: "play none none reverse",
          },
        });
  
        // animate letters
        tl.fromTo(
          letters,
          { y: 50, opacity: 0, filter: "blur(5px)" },
          {
            y: 0,
            opacity: 1,
            filter: "blur(0px)",
            stagger: 0.08,
            ease: "back.out(1.7)",
            duration: 0.6,
          }
        );
  
        // animate border underline
        tl.fromTo(
          borderRef.current,
          { width: "0%" },
          { width: "100%", duration: 0.8, ease: "power2.out" },
          "-=0.3" // overlap a bit with last letters
        );
      }
    }, []);
  
    const text = "MY SKILLS";
  
  return (
    <div className="h-fit w-full relative pb-20">
      <div className="flex flex-col text-xs h-full w-full px-5">
        <div className="flex flex-col">
        <div className="flex flex-col">
          {/* Animated Heading */}
          <div className="relative w-full lg:w-[50%] my-2">
            <div
              ref={headingRef}
              className="text-[40px] lg:text-[80px] syncopate-regular text-gray-300 flex flex-wrap overflow-hidden"
            >
              {text.split("").map((char, i) => (
                <span
                  key={i}
                  className="heading-char inline-block overflow-hidden"
                >
                  {char === " " ? "\u00A0" : char}
                </span>
              ))}
            </div>
            {/* Border that animates */}
            <div
              ref={borderRef}
              className="h-[2px] bg-red-800 absolute left-0 bottom-0"
              style={{ width: "0%" }}
            />
          </div>
        </div>
          <div className="lg:p-8 text-[12px] lg:text-sm play-regular text-gray-400 w-full flex flex-col gap-5">
            <div className="max-w-[730px] h-fit ml-auto">
              {"I enjoy "}
              <span className="font-bold text-red-300/60">
                solving problems
              </span>
              {" and bringing ideas to life — from building smooth "}
              <span className="font-bold text-red-300/60">UIs</span>
              {" to wiring up the "}
              <span className="font-bold text-red-300/60">logic</span>
              {" behind them. I like "}
              <span className="font-bold text-red-300/60">experimenting</span>
              {", learning new "}
              <span className="font-bold text-red-300/60">tools</span>
              {", and making projects that feel both "}
              <span className="font-bold text-red-300/60">clean</span>
              {" and "}
              <span className="font-bold text-red-300/60">practical</span>
              {"."}
            </div>

            <div className="max-w-[730px] h-fit ml-auto">
              {"My focus is on creating reliable "}
              <span className="font-bold text-red-300/60">full-stack apps</span>
              {", experimenting with "}
              <span className="font-bold text-red-300/60">
                real-time features
              </span>
              {", handling "}
              <span className="font-bold text-red-300/60">auth flows</span>
              {" and "}
              <span className="font-bold text-red-300/60">
                state management
              </span>
              {", and making sure every project balances "}
              <span className="font-bold text-red-300/60">performance</span>
              {" with a polished "}
              <span className="font-bold text-red-300/60">UI/UX</span>
              {"."}
            </div>
          </div>

          <div className="h-fit w-full flex flex-col items-center justify-center mt-15">
            <div className="w-full lg:w-250 max-w-500 h-fit flex flex-col lg:flex-row gap-2 lg:gap-8 lg:items-center items-start border-b-1 border-gray-400/20 py-5">
              <div className="border-1 syncopate-regular border-red-800 rounded-[100%] size-8 lg:size-20 text-sm lg:text-[35px] font-light text-gray-200 flex justify-center items-center">
                {"1"}
              </div>
              <div className="play-regular text-2xl lg:text-[35px] font-light text-gray-200 max-w-80">
                {"Front-end Developement"}
              </div>
              <div className="flex flex-col gap-6 ml-auto max-w-130">
                <div className="text-[11px] lg:text-sm play-regular text-gray-400 h-full flex ">
                  {
                    "This involves creating the user-facing side of applications, making sure everything looks clean, feels responsive, and works smoothly across devices."
                  }
                </div>
                <div className="flex gap-5 flex-wrap flex-col max-h-50 rounded-md justify-center bg-linear-to-br from-red-800/20 via-red-900/20 to-red-800/30 p-5 text-sm h-fit">
                  <span className="text-red-200/70">React</span>
                  <span className="text-red-200/70">Next.js</span>
                  <span className="text-red-200/70">Zod</span>
                  <span className="text-red-200/70">Photoshop </span>
                  <span className="text-red-200/70">Shadcn</span>
                  <span className="text-red-200/70">Material Ui</span>
                  <span className="text-red-200/70">GSAP</span>
                  <span className="text-red-200/70">
                    Framer Motion
                  </span>
                  <span className="text-red-200/70">
                    Tailwind CSS
                  </span>
                </div>
              </div>
            </div>

            <div className="w-full lg:w-250 max-w-500 h-fit flex flex-col lg:flex-row gap-2 lg:gap-8 lg:items-center items-start border-b-1 border-gray-400/20 py-5">
              <div className="border-1 syncopate-regular border-red-800 rounded-[100%] size-8 lg:size-20 text-sm lg:text-[35px] font-light text-gray-200 flex justify-center items-center">
                {"2"}
              </div>
              <div className="play-regular text-2xl lg:text-[35px] font-light text-gray-200 max-w-80">
                {"Back-end Development"}
              </div>
              <div className="flex flex-col gap-6 ml-auto max-w-130">
                <div className="text-[11px] lg:text-sm play-regular text-gray-400 h-full flex ">
                  {
                    "This involves building server-side logic, managing databases, handling authentication, optimizing performance, and deploying scalable applications on cloud platforms."
                  }
                </div>
                <div className="flex gap-5 flex-wrap flex-col max-h-50 rounded-md justify-center bg-linear-to-br from-red-800/20 via-red-900/20 to-red-800/30 p-5 text-sm h-fit">
                  <span className="text-red-200/70">Node.js</span>
                  <span className="text-red-200/70">Express.js</span>
                  <span className="text-red-200/70">Prisma ORM</span>
                  <span className="text-red-200/70">Mongoose</span>
                  <span className="text-red-200/70">
                    Authentication
                  </span>
                  <span className="text-red-200/70">WebSockets</span>
                  <span className="text-red-200/70">MongoDB</span>
                  <span className="text-red-200/70">PostgreSQL</span>
                  <span className="text-red-200/70">AWS (S3)</span>
                  <span className="text-red-200/70">Docker</span>
                  <span className="text-red-200/70">Vercel</span>
                  <span className="text-red-200/70">Render</span>
                </div>
              </div>
            </div>

            <div className="w-full lg:w-250 max-w-500 h-fit flex flex-col lg:flex-row gap-2 lg:gap-8 lg:items-center items-start border-b-1 border-gray-400/20 py-5">
              <div className="border-1 syncopate-regular border-red-800 rounded-[100%] size-8 lg:size-20 text-sm lg:text-[35px] font-light text-gray-200 flex justify-center items-center">
                {"3"}
              </div>
              <div className="play-regular text-2xl lg:text-[35px] font-light text-gray-200 max-w-80">
                {"Languages"}
              </div>
              <div className="flex flex-col gap-6 ml-auto max-w-130">
                <div className="text-[11px] lg:text-sm play-regular text-gray-400 h-full flex ">
                  {
                    "Core programming and query languages I use to build strong foundations in both front-end and back-end systems."
                  }
                </div>
                <div className="flex gap-5 flex-wrap flex-col max-h-50 rounded-md justify-center bg-linear-to-br from-red-800/20 via-red-900/20 to-red-800/30 p-5 text-sm h-fit">
                  <span className="text-red-200/70">Java</span>
                  <span className="text-red-200/70">
                    SQL (Postgres)
                  </span>
                  <span className="text-red-200/70">JavaScript</span>
                  <span className="text-red-200/70">HTML/CSS</span>
                  <span className="text-red-200/70">TypeScript</span>
                </div>
              </div>
            </div>

            <div className="w-full lg:w-250 max-w-500 h-fit flex flex-col lg:flex-row gap-2 lg:gap-8 lg:items-center items-start border-b-1 border-gray-400/20 py-5">
              <div className="border-1 syncopate-regular border-red-800 rounded-[100%] size-8 lg:size-20 text-sm lg:text-[35px] font-light text-gray-200 flex justify-center items-center">
                {"4"}
              </div>
              <div className="play-regular text-2xl lg:text-[35px] font-light text-gray-200 max-w-80">
                {"Tools and Workflow"}
              </div>
              <div className="flex flex-col gap-6 ml-auto max-w-130">
                <div className="text-[11px] lg:text-sm play-regular text-gray-400 h-full flex ">
                  {
                    "Using modern tools to collaborate efficiently, optimize workflows, and ship faster with reliable development, testing, and design practices."
                  }
                </div>
                <div className="flex gap-5 flex-wrap flex-col max-h-50 rounded-md justify-center bg-linear-to-br from-red-800/20 via-red-900/20 to-red-800/30 p-5 text-sm h-fit">
                  <span className="text-red-200/70">Git GitHub</span>
                  <span className="text-red-200/70">CI/CD</span>
                  <span className="text-red-200/70">Zustand</span>
                  <span className="text-red-200/70">JWT Auth</span>
                  <span className="text-red-200/70">VS Code</span>
                  <span className="text-red-200/70">Postman</span>
                  <span className="text-red-200/70">Docker</span>
                  <span className="text-red-200/70">Figma</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
